...
async fn router_state_changes_after_judge_series() {
    ...
    for (index, score) in scores.iter().enumerate() {
        ...
        dispatch::route("memory_search", &state, json!({
            "query": format!("learning context number {index}"),
            "limit": 5,
        })).await.expect("search should succeed");

        dispatch::route("memory_judge", &state, json!({
            "memory_id": &memory_id,
            "score": *score,
    ...
    let final_search = dispatch::route("memory_search", &state, json!({
        "query": "learning context",
        "limit": 10,
    })).await.expect("post-learning search should succeed");
    let results = final_search.as_array().expect("results array");
    assert!(!results.is_empty(), "search must return results after learning");
}
...
async fn feedback_tracking_records_searches() {
    ...
    let status_before = dispatch::route("memory_status", &state, json!({}))
        .await.expect("status should succeed");
    let pending_before = status_before["pending_judgments"].as_u64().expect("pending_judgments");
    assert!(pending_before > 0, "searches without judgment should create pending entries");
    ...
    let status_after = dispatch::route("memory_status", &state, json!({}))
        .await.expect("status after judge should succeed");
    let pending_after = status_after["pending_judgments"].as_u64().expect("pending_judgments");
    assert!(pending_after <= pending_before, "judging should not increase pending count");
}
