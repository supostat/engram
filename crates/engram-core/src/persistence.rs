...
fn load_from_disk(path: &Path) -> Result<IndexSet, CoreError> {
    let file =
        fs::File::open(path).map_err(|error| CoreError::IndexCorrupted(error.to_string()))?;
...
pub fn extract_embeddings_from_memory(
    memory: &engram_storage::Memory,
...
fn extract_embedding(
    memory: &engram_storage::Memory,
) -> Option<engram_embeddings::ThreeFieldEmbedding> {
