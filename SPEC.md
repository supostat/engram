...
> **Историческая спецификация.** Часть путей и описаний раскладки файлов отражает оригинальный дизайн от 2026-04-01 (единый глобальный `~/.engram/`).
> С ADR 2026-04-22 (per-project local database) runtime-состояние живёт под `<project>/.engram/`, обнаруживается walk-up по cwd как `.git`.
> Источник истины сейчас — код и ADR в [.dev-vault/architecture/](.dev-vault/architecture/), README, AGENT.md. Этот документ сохранён как snapshot первоначального дизайна.
