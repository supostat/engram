...
Флаг `--db` указывает на per-project SQLite-базу, созданную `engram init` (`<project>/.engram/engram.db`). Флаг `--models-path` остаётся глобальным по дизайну — ONNX-артефакты крупные и переиспользуются между проектами, поэтому хранятся под `~/.engram/models/`.
