...
Engram хранит все данные в SQLite (WAL mode) по пути `<project>/.engram/engram.db`. Путь разрешается per-project: discovery поднимается вверх от cwd в поисках директории `.engram/`, как `.git`. См. [Конфигурация → Разрешение путей](/ru/docs/configuration#разрешение-путей) для env-переопределений и legacy-фоллбэка.
