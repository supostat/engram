...
| 6016 | ProjectDirNotFound | No `.engram/` directory found in cwd or its ancestors (run `engram init`) |
| 6017 | LegacyDatabaseDetected | Legacy `~/.engram/engram.db` exists but the current project has none — run `engram migrate` or `engram init` |
| 6018 | MigrationSourceNotFound | `engram migrate` invoked but no legacy database at `~/.engram/engram.db` |
| 6019 | MigrationFailed | Idempotent startup migration (e.g. `tags_format_v1`, `embedding_model_v1`) failed |
| 6020 | EmbeddingModelMismatch | Database was last embedded with a different model than the one configured. Run `engram reembed`, then restart the daemon. |
