...
Engram stores all data in SQLite (WAL mode) at `<project>/.engram/engram.db`. The path is resolved per-project: discovery walks up from cwd looking for `.engram/`, like `.git`. See [Configuration → Path resolution](/en/docs/configuration#path-resolution) for env overrides and the legacy fallback.
