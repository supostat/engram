...
- **HyDE** — opt-in via `embedding.hyde_threshold > 0` (disabled by default). When enabled, LLM generates a hypothetical memory and the embedding is built from the hypothesis instead of the raw query.
